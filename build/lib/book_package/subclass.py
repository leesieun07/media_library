...
class DetailedBookSearch(BookSearchSystem):
    """부모 클래스를 상속받아 API 검색 기능을 구현한 자식 클래스.

    :ivar location: 서점 또는 라이브러리 위치
    :ivar api_key: 카카오 REST API 키
    """

    def __init__(self, system_name: str, location: str,
        api_key: str = "") -> None:
        """super()를 활용하여 부모를 초기화하고 API 키 속성을 추가합니다.

        :param system_name: 검색 시스템 이름
        :param location: 서점 또는 라이브러리 위치
        :param api_key: 카카오 도서 검색 API 키 (선택 사항)
        """
    ...
    def search_via_api(self, keyword: str) -> List[Dict[str, str]]:
        """카카오 도서 검색 API를 사용하여 실시간으로 책을 검색합니다.

        :param keyword: 검색할 도서 키워드
        :return: 정제된 도서 목록
        """
        cleaned_keyword = clean_keyword(keyword)
        if not cleaned_keyword or not self.api_key:
            return []

        url = "https://dapi.kakao.com/v3/search/book"
        headers = {"Authorization": f"KakaoAK {self.api_key}"}
        params = {"query": cleaned_keyword, "size": 5}
